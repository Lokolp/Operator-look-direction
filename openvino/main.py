...
class LandmarkDetection:
    ...
    def calc_eye_sizes(self, face, _X):
        self.eye_sizes = [abs(int((self.lm[0][_X] - self.lm[1][_X]) * face.shape[1])), abs(int(
            (self.lm[3][_X] - self.lm[2][_X]) * face.shape[1]))]  # eye size in the cropped face image
        return self.eye_sizes

    def calc_eye_centers(self, face, _X, _Y):
        self.eye_centers = [[int(((self.lm[0][_X] + self.lm[1][_X]) / 2 * face.shape[1])),
                             int(((self.lm[0][_Y] + self.lm[1][_Y]) / 2 * face.shape[0]))],
                            [int(((self.lm[3][_X] + self.lm[2][_X]) / 2 * face.shape[1])), int(((self.lm[3][_Y] +
                                                                                                 self.lm[2][_Y]) / 2 *
                                                                                                face.shape[
...
class GazeEstimation:
    ...
    def calc_gaze_lines(self, eye_centers, _X, _Y, xmin, ymin, gaze_vec_norm):
        for i in range(2):
            ...
            coord2 = (eye_centers[i][_X] + xmin + int((gaze_vec_norm[0] + 0.) * 3000),
                      eye_centers[i][_Y] + ymin - int((gaze_vec_norm[1] + 0.) * 3000))
    ...
    def draw_gaze_lines_and_centre(self, out_img, laser_flag, calib_delta_x, calib_delta_y):
        ...
        try:
            ...
            gaze_centre_t = (int(self.gaze_centre[0] + calib_delta_x), int(self.gaze_centre[1] + calib_delta_y))
...
def main():
    ...
    calib_delta_x = 0  # 466.3179012345679
    calib_delta_y = 0  # -364.787037037037
    ...
    while True:
        ...
        for obj in res_det[face_detect.out_name_det][0][0]:  # obj = [ image_id, label, conf, xmin, ymin, xmax, ymax ]
            if obj[2] > 0.75:  # Confidence > 75%
                ...
                lm = landmark_detect.find_landmarks(face, _W,
                                                    _H)  # [[left0x, left0y], [left1x, left1y], [right0x, right0y], [right1x, right1y] ]
                ...
                for i in range(2):
                    # Crop eye images
                    x1, x2, y1, y2 = landmark_detect.crop_eye(i, _X, _Y, face, gaze_estim.input_shape_gaze[_W],
                                                              gaze_estim.input_shape_gaze[_H])

                    if boundary_box_flag == True:
                        landmark_detect.draw_eye_bounding_box(out_img, x1, x2, y1, y2, xmin, ymin)

                    landmark_detect.rotate_eyes(roll, i, gaze_estim.input_shape_gaze[_W],
                                                gaze_estim.input_shape_gaze[_H])

                gaze_vec_norm = gaze_estim.calc_gaze_vec(landmark_detect.eyes, roll, pitch,
                                                         yaw)  # normalize the gaze vector
                print(gaze_vec_norm)
        ...
        gaze_estim.draw_gaze_lines_and_centre(out_img, laser_flag, calib_delta_x, calib_delta_y)
